...
class App {
	...
	private pocketCam!: ArcRotateCamera;
	private scoringCam!: ArcRotateCamera;
	...
	private async init() {
		...
		this.test();
		// new Game(assets);
		// const scoring = n	ew Scoring(this.scene);
	...
	private async createScene() {
		...
		// 캠 설정
		this.boardCam = this.createBoardCam();
		this.pocketCam = this.createPocketCam();
		this.scoringCam = this.createScoringCam();
		this.scene.activeCameras?.push(this.boardCam, this.pocketCam, this.scoringCam);
		...
		this.scene.onPointerObservable.add((evt) => {
			if (evt.type == PointerEventTypes.POINTERMOVE) {
				if (this.scene.pointerX < window.innerWidth * 0.2) {
					this.scene.cameraToUseForPointers = this.pocketCam;
				} else if (this.scene.pointerX > window.innerWidth * 0.9) {
					this.scene.cameraToUseForPointers = this.scoringCam;
				} else {
